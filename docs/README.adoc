...
- Intercambiables: Despliegue de actualizaciones en caliente.
...
- `-d` Lanza el contenedor en modo _detached_
...
* postgres: Base de datos PostgreSQL (relational)
* node: Node.js
...
[TIP]
====
Podemos eliminar todos los contenedores creados a partir de una imagen con la secuencia de comandos siguiente (p. ej. eliminar todos los contenedores creados a partir de una imagen `wordpress`)
...
* Se crean en un contexto (normalmente añadiendo archivos del directorio de trabajo del host a la imagen -p. ej. el código fuente de la aplicación)
...
Vamos a construir un contenedor que incluya de forma estática una aplicación (p. ej. la última versión de la aplicación). El proceso a seguir es:
...
[TIP]
====
También podemos hacer uso de la evaluación de órdenes con apóstrofes para obtener el path actual y añadirle solo la carpeta `html`.
...
* Al hacer `push` las capas que ya estén subidas no se vuelven a subir. Cuando una instrucción del `Dockerfile` cambia una capa, invalida al resto y hay que volver a generar las instrucciones de las capas restantes. Por tanto, colocaremos antes en el `Dockerfile` lo que menos cambie.
...
* Usaremos un archivo `docker-compose.yml` para configurar los _servicios_ de la aplicación. Los servicios son las partes de la aplicación (p. ej. un servicio para el almacenamiento de los datos y otro para el front-end)
...
1. Crear el archivo `docker-compose.yml` con los servicios de la aplicación (p. ej. php y mysql)
2. Construir y lanzar el entorno en modo _detached_ con `docker-compose up -d`
...
* Un archivo `docker-compose.yml` que configura dos servicios. Un servicio para almacenamiento de datos con MySQL y otro servicio para el front-end PHP de la aplicación.
...
Para ejecutar _Portainer_ con una carpeta local para el almacenamiento de los datos de Portainer (p. ej. usuarios) ejecutaríamos el comando siguiente:
...
Rancher dispone actualmente de dos versiones con funcionalidades diferentes: 1.6 y 2.0
...
* También, ajustaremos la cantidad de los recursos del host que se dedican a la ejecución de las réplicas.
...
* Docker Swarm propone que algunos de los conceptos de contenedores en un solo host sean válidos para convertirlo en un cluster (p. ej. redes overlay VXLAN)
...
* Hay dos tipos de nodos: _manager_ y _worker_.
    ...
    - Los nodos _worker_ solo se encargan de ejecutar contenedores. De forma predeterminada, al definir un _manager_ también es _worker_.

[NOTE]
====
Podemos hacer que los _managers_ no sean _workers_ haciendo que su disponibilidad sea `drain`
...
Esto crea un _swarm_ de un nodo configurando como _manager_ la máquina sobre la que se ha ejecutado. Además, muestra las instrucciones para añadir nuevos nodos _worker_ o _manager_ al _swarm_ creado.
...
* `php` Tendrá 3 réplicas. A cada una de ellas le limitamos los recursos al 10% de uso de la CPU del host en el que se ejecuta el contenedor (también se pueden indicar qué núcleos usar) y 50 MB de RAM.
...
Para escalar con la técnica de _re-despliegue_, editar el archivo `docker-compose.yml` con el nuevo número de réplicas y volver a hacer el despliegue con
...
* Administrar _swarms_ aprovisionándolos y añadiéndoles nodos
...
Para acceder a las máquinas creadas y ver que aún no tienen contenedores  creados debemos cargar las variables de entorno de la que vayamos a usar (p. ej. `nodo1`).
...
Para finalizar vamos a ver cómo desalojar un nodo (p. ej. debido a una operación de mantenimiento en uno de los servidores del _swarm_).
...
.Elección de OpenStack como infraestructura para despliegue de un cluster de contenedores
image::./Rancher20.png[]
...
* Dockerizar con cabeza
    - Comenzamos pasando todo nuestro sistema o MV a un contenedor Docker. Con solo eso ya conseguimos ejecutar nuestro sistema en distintas máquinas con distintos SO y configuraciones.
    - No intentar pasar de una vez de aplicación monolítica a micro servicios diminutos
...
<4> Usa la política de balanceo _least connections_ en lugar de _round robin_ la cual es la predeterminada
